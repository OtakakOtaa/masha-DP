...
using ARTnGAME.LIQUA_Volumetric_Fluids.MODULES.Unity_Simple_Liquid.Scripts.Utils;
using UnityEngine;
...
namespace ARTnGAME.LIQUA_Volumetric_Fluids.MODULES.Unity_Simple_Liquid.Scripts
{
    ...
    public class LiquidContainer : MonoBehaviour
    {
        ...
        //v0.1
        public float fillScaling = 0.3f;
        public float fillOffset = -0.28f;
        ...
        public Vector3 SurfaceLevel
        {
            ...
            set
            {
                ...
                if (MaterialInstance)
                {
                    //MaterialInstance.SetVector(SurfaceLevelID, value); //v0.1
                    if (debug)
                    {
                        Debug.Log("Level = " + ((value.y - fillOffset) * fillScaling).ToString());
                    }
                    float colliderY = 0;
                    if(GetComponent<CapsuleCollider>() != null)
                    {
                        colliderY = GetComponent<CapsuleCollider>().center.y;
                    }
                    MaterialInstance.SetFloat(SurfaceLevelID, (value.y  - fillOffset - colliderY) * fillScaling); //v0.1
                }
            }
        }
        ...
        private static int SurfaceLevelID =Shader.PropertyToID("_fill_Level"); //Shader.PropertyToID("_SurfaceLevel");//v0.1
        ...
        public Material MaterialInstance
        {
            get
            {
                ...
                if (materialInstance == null || liquidRender.sharedMaterial != materialInstance)
                {
                    ...
                    {
                        var shader = matShader;// Shader.Find("Liquid/SimpleLiquidShader"); //Shader.Find("Liquid/SimpleLiquidShader");
