...
namespace ZeppelinGames
{
    public class AudioEditor : EditorWindow
    {
        ...
        void DrawRightGroup()
        {
            ...
            {
                DrawAudioMarker(new Rect((audioMarkers[n].markerTime * (rightGroup.width / audioClip.length)), audioLayers.IndexOf(audioMarkers[n].audioLayer) * markerSize.y, markerSize.x, markerSize.y), audioMarkers[n]);
            }
            GUI.EndScrollView();
            GUI.EndGroup();
