Shader "LIQUA/FSVE/VolumeFireMarching" 
{
	Properties 
	{
		...
		pass
		{
			...
			*/
			...
			float4 Frag(v2f IN) : COLOR
			{
				...
				//NEW FIRE
				//float sceneZA = LinearEyeDepth(tex2D(_CameraDepthTexture, IN.ScreenPos.xy / IN.ScreenPos.w));
				...
				for (int i = 0; i < NUM_STEPS; ++i)
				{
					...
					//FIRE
					/*float3 startTRAN = start;
					float dist = length((float3(startTRAN.x * _boxScales.x, startTRAN.y * _boxScales.y, startTRAN.z * _boxScales.z)
						- float3(rayStart.x * _boxScales.x, rayStart.y * _boxScales.y, rayStart.z * _boxScales.z)) * 1 * _FireIntensity);
					if (dist > sceneZA)
					{
					}
					else {
				...
				//float4 both = fire + smoke;
				//both.rgb = pow(both.rgb, _FireContrast) * 2 * 5;// _FireIntensity;
	...
	//CustomEditor "Artngame.LIQUA.FSVE.VolumeShaderEditor"// Custom editor displaying standard blend mode combinations 
}