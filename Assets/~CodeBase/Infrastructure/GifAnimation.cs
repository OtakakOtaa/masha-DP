...
namespace _CodeBase.Infrastructure
{
    public sealed class GifAnimation : MonoBehaviour
    {
        ...
        private void Bake()
        {
            ...
            var gfInfo = GifAnimationDecoder.Process(Filename);
            _mFrames = gfInfo.Frames.ToList();
            _mFrameDelay = gfInfo.FramesDelays.ToList();
        ...
        public async UniTask Play(Color? color = null, float? duration = null, GifInfo gifInfo = default, bool? loopFlag = null)
        {
            _spriteRenderer ??= GetComponent<SpriteRenderer>();
            _image ??= GetComponent<Image>();
            ...
            if (gifInfo != default)
            {
                _mFrames = gifInfo.Frames.ToList();
                _mFrameDelay = gifInfo.FramesDelays.ToList();
            }
        ...
        private void UpdateGif()
        {
            ...
            if (_mTime >= _mFrameDelay[_mCurFrame])
            {
                ...
                if (_mCurFrame + 1 == _mFrameDelay.Count && _oneLoopPlayed != null && _loopFlag == false)
                {
